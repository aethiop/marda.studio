(function () {
			...
			function ack(msg, node) {
				...
				node &&
					(function go() {
						...
						console.STAT &&
							console.STAT(S, +new Date() - S, "got in");
						if (!tmp) {
							return;
						}
						setTimeout.turn(go);
			...
			if (u === change) {
				...
				setTimeout.each(
					...
					function (get, sat) {
						...
						if (link) {
							delete (root.$.get(link).get(get)._.echo || "")[
								sat.id
							];
						}
				...
				function any(msg, eve, f) {
					...
					if (u === opt.stun) {
						if ((tmp = root.stun) && tmp.on) {
							...
							!test.run &&
								sat &&
								tmp.on("" + sat.id, (test = {})); // linked node stunned?
							if (any.id > test.run) {
	...
	USE(function (module) {
		...
		function Mesh(root) {
			...
			opt.max =
				opt.max ||
				(opt.memory ? opt.memory * 999 * 999 : 300000000) * 0.3;
				...
				if ("[" === tmp) {
					parse(raw, function (err, msg) {
						...
						(function go() {
							...
							console.STAT &&
								console.STAT(S, +new Date() - S, "hear loop");
			...
			(function () {
				...
				var say = (mesh.say = function (msg, peer) {
					...
					if (!peer || !peer.id) {
						...
						console.STAT &&
							console.STAT(S, +new Date() - S, "peer keys");
						(function go() {
							var S = +new Date();
							...
							console.STAT &&
								console.STAT(S, +new Date() - S, "say loop");
							if (!pl.length) {
								return;
							}
							puff(go, 0);
					...
					setTimeout(function () {
						console.STAT &&
							console.STAT(ST, +new Date() - ST, "0ms TO");
						flush(peer);
				...
				mesh.raw = function (msg, peer) {
					...
					if ((put = meta.$put)) {
						...
						json(tmp, function (err, raw) {
							...
							console.STAT &&
								console.STAT(S, +new Date() - S, "say slice");
						});
						return;
					}
					json(msg, res);
		...
		Gun.on("opt", function (root) {
			...
			function reconnect(peer) {
				...
				peer.retry =
					...
					(-peer.tried + (peer.tried = +new Date()) < wait * 4
						? 1
						: 0);
	...
	USE(function (module) {
		...
		Gun.on("create", function lg(root) {
			...
			try {
				disk = lg[opt.prefix] =
					lg[opt.prefix] ||
					JSON.parse(store.getItem(opt.prefix)) ||
